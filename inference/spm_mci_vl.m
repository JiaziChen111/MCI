function [vl] = spm_mci_vl (M,U,Y)
% Variational Laplace inversion of nonlinear models
% FORMAT [vl] = spm_mci_vl (M,U,Y)
%
% This is the same code as in spm_nlsi_GN.m but returns
% more information.
...
%
%
% vl    Output Data Structure
%       .Ep     Posterior mean of parameters
%       .Cp     Posterior covariance of parameters
%       .hE     Prior mean of log-precision of obs noise
%       .hC     Prior covariance of log-precision of obs noise
%       .Eh     Posterior mean of log-precision of obs noise
%       .Ch     Posterior covariance of log-precision of obs noise
%       .F      Log model evidence
%       .L      Contributions to F; L(1) data fit, L(2) (-ve) complexity of
%               parameters, L(3) (-ve) complexity of obs noise precisions
%       .dFdp   Gradient of F 
%       .dFdpp  Curvature of F
%       .k      Number of iterations used in optimisation
%
...
% Karl Friston and v. minor edits W. Penny
% $Id$
...
% outputs
%--------------------------------------------------------------------------
vl.Ep = spm_unvec(spm_vec(pE) + V*C.p(ip),pE);
vl.Cp = V*C.Cp(ip,ip)*V';
vl.hE = hE;
vl.hC = spm_inv(ihC);
vl.Eh = C.h;
vl.Ch = Ch;
