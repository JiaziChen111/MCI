...
%                           otherwise
%       .beta               Inverse temperature (default=1)
%                           
% U     Inputs
% Y     Data
%
...
try lgv_update=mcmc.lgv_update; catch lgv_update='default'; end
...
while (i <= maxits),
    ...
    if i==1
        ...
        pos = spm_mci_proposal (M,U,curr.mu,curr.Cp);
    end
       
    % Quantities re proposal
    prop.pos=pos;
    prop.h=h;
    prop.update=lgv_update;
    prop = spm_mci_compute_proposal (prop,M,U,Y,beta);
    ...
    delta=1;nstab=0;
    prop.mu=pos+prop.dpos;
    while stab==0
        prop.mu = pos+delta*prop.dpos;
        stab = feval(M.stabfun,prop.mu,M,U);
        delta = delta/2;
        nstab = nstab + 1;
        if nstab > 16
            disp('Abandoning step due to numerical instability');
            stab=1;
            prop.mu=pos;
        end
    end
    ...
    if (i > update_obs_step & update_obs_noise) | (restart & update_obs_noise)
        ...
        % Sample covariance weighted by beta for 
        % AIS algorithm
        noise.DN=noise.D0+beta*0.5*NT*cov(err);
