...
%           .S      covariance of (fixed/tuned) proposal density
...
% Project covariance of sampling density back into original space
for j=1:J,
    dC=P{j}.Cs{1};
    dV=P{j}.Cs{2};
    P{j}.S=V*dV*diag(dC)*dV'*V';
    % Enforce symmetry
    for i=1:Np,
        for k=i+1:Np,
            P{j}.S(i,k)=P{j}.S(k,i);
...
if strcmp(anneal,'frozen')
    ...
    % Use posterior samples only
    logev.phm = spm_mci_phm (L(burn_in+1:end));
    % Record likelihoods of (all) samples
    D.like = L;
else
    logev.phm = [];
end
