function [els,names,grad] = mci_compare_gradients (model,cost,methods)
% Compare methods for gradient computation
% FORMAT [els,names,grad] = mci_compare_gradients (model,cost,methods)
%
...
figure;
if m(1) plot(dLdp(ind)); grad.dLdp=dLdp; end
hold on
if m(2) plot(dLdp_sun(ind),'c'); grad.dLdp_sun=dLdp_sun; end
if m(3) plot(dLdp_adj(ind),'r'); grad.dLdp_adj=dLdp_adj; end
if m(4) plot(dLdp_adj_sun(ind),'m'); grad.dLdp_adj_sun=dLdp_adj_sun; end
if m(5) plot(dLdp_finite(ind),'k'); grad.dLdp_fd=dLdp_finite; end
xlabel('p');
ylabel('dLdp');
grid on
legend(names);
